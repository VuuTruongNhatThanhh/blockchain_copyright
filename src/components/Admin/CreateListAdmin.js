  ...
  useEffect(() => {
    const loadWorks = async () => {
      ...
      try {
        ...
        let tempWorks = [];
        let countStatus0 = 0; // Biến đếm số lượng tác phẩm có status là 0
        for (let i = 0; i < workCount; i++) {
          ...
          if (processedWork.status === 0) { // Chỉ thêm vào danh sách nếu status là 0
            ...
            sessionStorage.setItem('totalCreateWorks', parseInt(countStatus0));
            

        }
        setWorks(tempWorks);
        setTotalCreateWorks(countStatus0); // Số lượng tác phẩm có status là 0
...
const handleApproveWork = async (work) => {
  ...
  try {
    ...
    .on('transactionHash', function(hash){
      ...
      alert('Duyệt đăng ký tác phẩm thành công');
    })
...
<button style={{height:'42px', width:'147px'}}  onClick={() => handleApproveWork(work)}>Duyệt đăng ký</button>
<button style={{backgroundColor:'#d72727',height:'42px'}} onClick={() => handleHiddenWork(work)}>Ẩn</button>
  ...
  return (
    <div className="worklist-container">
      <h4 style={{ marginBottom: '23px' }}>Các đăng ký bản quyền cần duyệt</h4>
      <input
        type="text"
