  ...
  useEffect(() => {
    const loadWorks = async () => {
      ...
      try {
        ...
        let tempWorks = [];
        let countStatus1 = 0; // Biến đếm số lượng tác phẩm có status là 0
        for (let i = 0; i < workCount; i++) {
          ...
          if (processedWork.status === 1) {
            tempWorks.push(processedWork);
            countStatus1++;
            
            
          }
          sessionStorage.setItem('totalWorksUpdate', parseInt(countStatus1));
        }
        setWorks(tempWorks);
...
<button style={{height:'42px', width:'160px'}}  onClick={() => handleApproveWork(work)}>Duyệt chỉnh sửa</button>
...
const handleHiddenWork = async (work) => {
 ...
 const handleApproveWork = async (work) => {
    ...
    try {
      ...
      .on('transactionHash', function(hash){
        console.log('Transaction hash:', hash);
        alert('Duyệt chỉnh sửa thành công');
      })
      .on('confirmation', function(confirmationNumber, receipt){
        if(confirmationNumber === 1) {
          console.log('Transaction confirmed');
  ...
  return (
    <div className="worklist-container">
      <h4 style={{ marginBottom: '23px' }}>Các tác phẩm được chỉnh sửa</h4>
      <input
        type="text"
...
export default UpdatelistAdmin;
